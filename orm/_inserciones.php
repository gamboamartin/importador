...
class _inserciones{
    ...
    final public function aplica_inserciones(array $imp_destino, PDO $link){
        ...
        $ejecuciones = $this->ejecuta_inserciones(imp_database_id: $imp_destino['imp_database_id'], link: $link, name_model: $name_model, rows: $rows);
        if(errores::$error){
    ...
    private function ejecuta_inserciones(int $imp_database_id,PDO $link, string $name_model, array $rows){
     ...
     */
    private function inserta_destino(modelo $destino, array $row): array|stdClass
    {
        $retorno = new stdClass();
