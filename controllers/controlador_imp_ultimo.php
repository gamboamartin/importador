...
class controlador_imp_ultimo extends _ctl_parent_sin_codigo {

    public stdClass|array $imp_ultimo = array();
    private imp_ultimo_html $html_local;
    ...
    public function __construct(PDO $link, html $html = new html(), stdClass $paths_conf = new stdClass()){
        ...
        $html_ = new imp_ultimo_html(html: $html);
        ...
        $this->titulo_lista = 'Ultimos';

        if(isset($this->registro_id) && $this->registro_id > 0){
            $imp_ultimo = (new imp_ultimo($this->link))->registro(registro_id: $this->registro_id);
            if(errores::$error){
                $error = $this->errores->error(mensaje: 'Error al obtener imp_ultimo',data:  $imp_ultimo);
                print_r($error);
                exit;
            }
            $this->imp_ultimo = $imp_ultimo;
        }
    ...
    public function alta(bool $header, bool $ws = false): array|string
    {
        ...
        $keys_selects = $this->key_select(cols:6, con_registros: true,filtro:  array(), key: 'imp_destino_id',
            keys_selects: array(), id_selected: -1, label: 'Destino');
        if(errores::$error){
    ...
    public function modifica(bool $header, bool $ws = false, array $keys_selects = array()): array|stdClass
    {
        ...
        $keys_selects = $this->key_select(cols:6, con_registros: true,filtro:  array(), key: 'imp_destino_id',
            keys_selects: array(), id_selected: $this->registro['imp_destino_id'], label: 'Destino');
        if(errores::$error){
            return $this->retorno_error(mensaje: 'Error al maquetar key_selects',data:  $keys_selects, header: $header,ws:  $ws);
        }
